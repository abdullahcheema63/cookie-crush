...
const int width = 900, height = 660; // i have set my window size to be 800 x 600
...
enum Cookies {
	CK_BISCUIT, CK_BURGER, CK_CROISSONT, CK_CUPCAKE, CK_DONUT, CK_STAR,abc
};
Cookies cookiesType[7]={CK_BISCUIT, CK_BURGER, CK_CROISSONT, CK_CUPCAKE, CK_DONUT, CK_STAR,abc};
...
//---------------------------traveersing cookies array--------------------------------------
		...
		for (int columnTraverse=0;columnTraverse<height/60-1;columnTraverse++)
		{
			if ((cookieArray[rowTraverse][columnTraverse]
					...
					& (cookieArray[rowTraverse][columnTraverse]
							== cookieArray[rowTraverse][columnTraverse + 2]))
			{
	...
	DrawString((width / 60) *2, height - 20, width, height,
